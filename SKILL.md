...
- `vaultgentic:vaultgentic_patch` — apply a strict unified diff to an existing note. Read first, then pass `expectedFileHash` from the read for concurrency safety; on hash mismatch or hunk failure, re-read and regenerate the diff
